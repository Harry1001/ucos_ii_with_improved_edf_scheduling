...
static  void  OS_SchedEDF(void);							/* Schedule tasks according to EDF */

static  void  readTimeWheel();
...
void  OSTimeTick (void)
{
    ...
    if (OSRunning == OS_TRUE) {
		...
		if ((OSPrioCur > 3u) && (OSPrioCur < OS_LOWEST_PRIO - 3)){				/* user created task */
			((EDF_TCB_DATA *)OSTCBCur->OSTCBExtPtr)->comp_time--;
			if (((EDF_TCB_DATA *)OSTCBCur->OSTCBExtPtr)->comp_time == 0){
				((EDF_TCB_DATA *)OSTCBCur->OSTCBExtPtr)->ddl += ((EDF_TCB_DATA *)OSTCBCur->OSTCBExtPtr)->p_value;
				((EDF_TCB_DATA *)OSTCBCur->OSTCBExtPtr)->comp_time = ((EDF_TCB_DATA *)OSTCBCur->OSTCBExtPtr)->c_value;
				((EDF_TCB_DATA *)OSTCBCur->OSTCBExtPtr)->end = OSTimeGet();
				OSTCBCur->OSTCBDly = ((EDF_TCB_DATA *)OSTCBCur->OSTCBExtPtr)->p_value - ((EDF_TCB_DATA *)OSTCBCur->OSTCBExtPtr)->end + ((EDF_TCB_DATA *)OSTCBCur->OSTCBExtPtr)->start;
				((EDF_TCB_DATA *)OSTCBCur->OSTCBExtPtr)->start += ((EDF_TCB_DATA *)OSTCBCur->OSTCBExtPtr)->p_value;
				heapDeleteMin(readyTaskHeap);
				readTimeWheel();
				printf("%d\tComplete\t%d\t%d\n", OSTimeGet(), OSTCBCur->OSTCBPrio, heapPeakMinTask(readyTaskHeap));

				insertWaitTask(waitTaskWheel, OSTCBCur);
				OS_Sched();
			}
			else {
				readTimeWheel();
				INT32U readyPrio = heapPeakMinTask(readyTaskHeap);
				if (readyPrio != OSTCBCur->OSTCBPrio){
					printf("%d\tPreempt\t\t%d\t%d\n", OSTimeGet(), OSTCBCur->OSTCBPrio, readyPrio);
				}
			}
		}
		else {																	/* system reserved task, with no EDF_TCB_DATA */
			readTimeWheel();
			INT32U readyPrio = heapPeakMinTask(readyTaskHeap);
			if (readyPrio != OSTCBCur->OSTCBPrio){
				printf("%d\tPreempt\t\t%d\t%d\n", OSTimeGet(), OSTCBCur->OSTCBPrio, readyPrio);
                   ...
                   INT16U   opt)
{
    ...
    if (ptcb != (OS_TCB *)0) {
		...
		EDF_TCB_DATA *tmp = (EDF_TCB_DATA *)ptcb->OSTCBExtPtr;
		if (tmp != (EDF_TCB_DATA *)0){		//must not be idle task
			if (tmp->p_value > maxPeriod){
				maxPeriod = tmp->p_value;
			}
			Heap_Data heap_data = {.prio = ptcb->OSTCBPrio, .deadline = tmp->ddl};
			heapInsert(&heap_data, readyTaskHeap);
		}
...
{
	OSPrioHighRdy = heapPeakMinTask(readyTaskHeap);
}

static void readTimeWheel()
{
	INT32U tick = OSTimeGet();
	OS_TCB *tcbList = getReleasedTask(waitTaskWheel, tick);
	while (tcbList != (OS_TCB *)0){
		Heap_Data heapData = { .prio = tcbList->OSTCBPrio, .deadline = ((EDF_TCB_DATA *)tcbList->OSTCBExtPtr)->ddl };
		OS_TCB *tmp = tcbList;
		tcbList = tcbList->TimeWheelNext;
		tmp->TimeWheelNext = (OS_TCB *)0;
		heapInsert(&heapData, readyTaskHeap);
	}
}
#endif
