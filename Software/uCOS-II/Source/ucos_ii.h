...
#define OS_ERR_EDF_DATA_INVALID       165u
...
/*
*********************************************************************************************************
*                                         EDF EXTENSIVE DATA AREA
*********************************************************************************************************
*/
#if OS_TASK_EDF_EN > 0u
typedef struct edf_tcb_data {
	INT32U c_value;							/* the task needs to consume c_value ticks					*/		
	INT32U p_value;							/* ... in each p_value ticks								*/		
	INT32U comp_time;						/* the remaining ticks to consume */
	INT32U ddl;								/* the deadline for the task now  */
	INT32U start;							/* the time when the task started, initialized to be when the task first run */
	INT32U end;								/* the time when the task complete in one cycle */
} EDF_TCB_DATA;
#endif
...
#if OS_TASK_EDF_EN > 0u
	struct os_tcb   *TimeWheelNext;			/* Pointer to next TCB in time wheel */
#endif
#if OS_TASK_CREATE_EXT_EN > 0u
#if defined(OS_TLS_TBL_SIZE) && (OS_TLS_TBL_SIZE > 0u)
...
#if OS_TASK_EDF_EN > 0u
OS_EXT  TimeWheel * waitTaskWheel;
OS_EXT  MinHeap * readyTaskHeap;
OS_EXT  INT32U  maxPeriod;
#endif
