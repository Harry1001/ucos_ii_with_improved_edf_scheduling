                        ...
                        INT16U   opt)
{
	...
	if ((prio > 3u)&&(prio < OS_LOWEST_PRIO-3)&&(pext == (EDF_TCB_DATA *)0)) {		/* If the EDF scheduling is enabled, then edf tcb data must be provided, except the task priority is reserved */
		return (OS_ERR_EDF_DATA_INVALID);
	}
#endif
